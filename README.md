...
Vanilla HTML/CSS/JS single-page portfolio served from `/docs`.

## GitHub Pages deployment
- Push changes to `main`.
- In GitHub, open Settings > Pages.
- Choose "Deploy from a branch", select `main`, and set the folder to `/docs`.
- Save; GitHub Pages will publish the site from the `docs` directory.

## Editing content
- Update `docs/content.json` to change text, links, lists, or section order.
- The page reads this JSON at load and renders dynamically; no build step is required.
- Assets live in `docs/assets/` (`style.css` and `main.js`); keep to system fonts and vanilla JS.
